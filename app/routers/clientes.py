...
def obtener_pedidos_de_cliente(cliente_id: int):
    cliente = repo.obtener_por_id(cliente_id)
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
    
    todos_los_pedidos = pedidos_repo.obtener_todos()
    pedidos_del_cliente = [p for p in todos_los_pedidos if p["id_cliente"] == cliente_id]
    return pedidos_del_cliente
